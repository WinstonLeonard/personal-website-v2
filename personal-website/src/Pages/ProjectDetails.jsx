...
import { CheckCircle2, Code2, Server, Wind, Zap, Palette, Github } from "lucide-react";
...
const ProjectDetails = () => {
    ...
    const demoVideoUrl = "https://mnuhdfulimayebkvchyh.supabase.co/storage/v1/object/public/techstack-icons//Screen%20Recording%202025-06-19%20164911.mp4";
    ...
    return (
        <>
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 py-10 md:py-14 text-left">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-start">
                    <div className="rounded-2xl bg-gradient-to-br from-slate-900/90 to-slate-800/90 border border-white/10 shadow-2xl p-6 md:p-8">
                        ...
                        <div className="mt-6 grid grid-cols-2 gap-4">
                            <div className="rounded-xl bg-slate-900/60 border border-white/10 p-4 text-center">
                                <div className="text-2xl md:text-3xl font-bold text-white">{project.technologies.length}</div>
                                <div className="text-xs md:text-sm text-gray-300 mt-1">Total Teknologi</div>
                            </div>
                            <div className="rounded-xl bg-slate-900/60 border border-white/10 p-4 text-center">
                                <div className="text-2xl md:text-3xl font-bold text-white">{project.features.length}</div>
                                <div className="text-xs md:text-sm text-gray-300 mt-1">Fitur Utama</div>
                            </div>
                        </div>

                        <div className="mt-6 flex flex-wrap items-center gap-4">
                            <a href="#" className="px-4 py-2 rounded-lg bg-blue-600/90 hover:bg-blue-600 text-white border border-white/10 shadow">Live Demo</a>
                            <a href="https://github.com/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-900/70 hover:bg-slate-900/90 text-gray-200 border border-white/10 shadow">
                                <Github size={18} />
                                <span>Github</span>
                            </a>
                        </div>
                    ...
                    <div className="rounded-2xl overflow-hidden bg-slate-900/60 border border-white/10 shadow-xl h-[24rem] md:h-[28rem]">
                        <Slider {...sliderSettings} className="h-full">
                            {[
                                ...project.images.map((src) => ({ type: "image", src })),
                                { type: "video", src: demoVideoUrl },
                            ].map((item, index) => (
                                <div key={index} className="bg-black/20 h-full">
                                    {item.type === "image" ? (
                                        <img
                                            src={item.src}
                ...
                <div className="mt-8 rounded-xl bg-gradient-to-br from-slate-900/80 to-slate-800/80 border border-white/10 shadow-xl p-6 md:p-7">
